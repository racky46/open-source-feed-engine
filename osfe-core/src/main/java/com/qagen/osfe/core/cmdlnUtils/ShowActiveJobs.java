 ...
 */
public class ShowActiveJobs extends ShowJobsBase {

  /**
   * Constructor
   */
  public ShowActiveJobs() {
    super();
  }

  /**
   * This method must be called in order to perform the tasks of retrieving
   * the set of feed jobs in an active state and echoed to the console.
   */
  public void execute() {
    final List<FeedJob> failedJobs = feedJobService.findByFeedJobStateId(FEED_JOB_STATE.active.getValue());
    showJobs(failedJobs);
  }
  ...
  public static void main(String[] args) {
    final ShowActiveJobs job = new ShowActiveJobs();
    job.execute();
  }
}
